...
class audioTrial:
    ...
    def __init__(self, audioPath,storyTimeDict, font, screen,currentStage,nextStage, output_control,entity=0,chunk_size=1024,verbose=0,practice=False):
                ...
                for evt in pygame.event.get():
                    ...
                    if evt.type == pygame.QUIT or (evt.type == pygame.KEYDOWN and evt.key == pygame.K_ESCAPE):
                        try:
                            ...
                            pass
                        return "exit"
                    # Ignore mouse events during audio playback (prevents accidental crashes)
                    if evt.type in (pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP, pygame.MOUSEMOTION):
                        continue
                    # Otherwise discard silently
            except Exception as e:
                try:
                    self.output_control.write(f"Ignored event processing error during audio: {e}")
                except Exception:
                    pass
        ...
        # ==== Thought Probe Part of a Trial =====
            ...
            try:
                for event in pygame.event.get():
                    ...
                    # Ignore mouse events here too (prevent accidental clicks causing errors)
                ...
                try:
                    self.output_control.write(f"Ignored probe event processing error: {e}")
                except Exception:
                    pass
