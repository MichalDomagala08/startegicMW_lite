...
def loadParticipant(path, name=""):
    ...
    + own       - my OWN events - Custom ONLY FOR ME! 
    + rawEye    - Raw Eye data
    + events    - EyeLink Sanctioned Events
    + gazeCoords -coordination of Eye data
    """
...
def plotTrialTrack(m,name,grand_df,ax,titl):
  ...
  ax.fill_between(trialN, 
                  mw_est_tr - grTr.std().loc['TRACKED'][name], 
                    mw_est_tr+   grTr.std().loc['TRACKED'][name], 
                  color='mediumslateblue', alpha=0.7, label='TRACKED (STD Envelope)')


  ax.fill_between(trialN, 
                  mw_est_utr - grTr.std().loc['UNTRACKED'][name], 
                    mw_est_utr+   grTr.std().loc['UNTRACKED'][name], 
                  color='moccasin', alpha=0.7, label='UNTRACKED (STD Envelope)')
...
def plot_pupil_vs_mw(m, currentDf, ind, ax=None,meas='Pupil'):
    """
